(ns hakkou.app
  "app-hakkou appview — reagent + re-frame, view built from jp-go-dds
  ...
  - `:app/route-count` and `:app/routes` now report what `wrangler.jsonc`
    actually declares (2 route patterns: `h4kk0001.etzhayyim.com/*` and
    `hakkou.etzhayyim.com/*`), not the stale `0` / `[]` the Svelte
    constant carried.
  ...
  components or ext-rules change) with:
    ...
    (spit \"public/index.html\"
          (page/->page {:title \"etzhayyim-project-hakkou\"
                         :lang \"ja\"
                         :description \"hakkou (発酵) — fermentation pipeline layer appview (reagent + re-frame + jp-go-dds).\"
                         :css (slurp (io/resource \"jp_go_dds/dds.css\"))}
                        [:div {:id \"app\"} \"etzhayyim-project-hakkou loading…\"]
...
(def default-db
  {:app/title "Ai etzhayyim Project Hakkou"
   :app/project "etzhayyim-project-hakkou"
   :app/name "etzhayyim-project-hakkou"
   :app/kind "cloudflare surface"
   :app/route-count 2
   :app/routes ["h4kk0001.etzhayyim.com/*" "hakkou.etzhayyim.com/*"]
   :app/vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_CAPABILITIES" "APP_DESCRIPTION"
              ...
              "APP_UI_TYPE"]
   :app/xrpc? false
   :app/relative-path "cljs/src/hakkou/app.cljs"})
